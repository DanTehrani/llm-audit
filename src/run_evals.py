...
async def answer_question(questions: list[str], contract_name: str, contract_path: str, past_questions_and_answers: str = "", call_metadata: dict = None):
...
def get_question_prompt(function_name: str, questions_and_answers: str, context: str, contract_overview: ContractOverview, contract_source_code: str):
    prompt = f"""
        ...
        You should either
        1. Ask up to 3 questions at a time to understand the function and the contract better to identify if there is any issue.
        Hypothesize what could be the issue and ask questions to understand the issue better.
        {{
            questions: ["<question1>", "<question2>", "<question3>"]
        }}
        2. If you have already identified an issue, return the following JSON object:
        {{
            "safe": false,
            "reason": "Your reason for finding an issue",
            "description": "Your description of the function's security"
        }}
        3. If you have not identified any issue, and you don't think there is anything else to ask, return the following JSON object:
        {{
            "safe": true,
...
async def audit_function(contract_path: str, function: FunctionContract, contract: Contract, contract_overview: ContractOverview, context: str, project_name: str): 
    ...
    for i in range(max_questions):
        ...
        prompt = get_question_prompt(function.name, questions_and_answers, context, contract_overview, contract_source_code)
        ...
        parsed_response = json.loads(response.output_text.replace("```json", "").replace("```", ""))

        if "questions" in parsed_response:
            start = time.time()
            answers, qa_usage = await answer_question(questions, contract.name, contract_path,  questions_and_answers, call_metadata)
            end = time.time()
        ...
        elif "safe" in parsed_response:
            final_answer = parsed_response

            result = FunctionAuditResult(
                project_name=project_name,
                function_name=function.name,
                safe=final_answer["safe"],
                reason=final_answer["reason"],
                description=final_answer["description"],
                cost="",
                proposedFix=final_answer["proposedFix"] if "proposedFix" in final_answer else None,
                failed=False
            )

            return result
        else:
            raise Exception(f"Invalid response: {response.output_text}")

    # If the code reach here, it means there have been too many questions asked and no issue has been found
    return FunctionAuditResult(
        project_name=project_name,
        function_name=function.name,
        safe=False,
        reason="Too many questions asked and no issue has been found",
        description="Too many questions asked and no issue has been found",
        cost="",
        proposedFix=None,
        failed=False
    )
...
async def judge_all_projects(audit_results: list[FunctionAuditResult]):
    ...
    for project in projects:
        ...
        try:
            judge_results = await judge_project(project_audit_results, project)
            all_judge_results.extend(judge_results)
        except Exception as e:
            print(colored(f"Error judging {project}: {e}", "red"))
            continue
...
async def run_evals():
    ...
    for project in projects_to_audit:
        ...
        audit_results = await audit_project(f"./dataset/{project_name}/flattened_with_impls", project["files"], project_name)
        all_audit_results.extend(audit_results)

    end_time = time.time()
    print(colored(f"Auditing all projects took {end_time - start_time} seconds", "green"))

    save_audit_results(all_audit_results, audit_id)

    start_time = time.time()
    judge_results = await judge_all_projects(all_audit_results)
    end_time = time.time()
    print(colored(f"Judging all projects took {end_time - start_time} seconds", "green"))
